...
export class AtsumaruScraper extends BaseScraper {
  constructor() {
    super({ siteId: 'atsumaru', siteName: 'Atsumaru', baseUrl: 'https://atsumaru.vip', requiresHeadless: false });
  }
  ...
  async getDetails(mangaId) {
    ...
    return formatMangaDetails({ id: mangaId, siteId: this.siteId, title: item.title || mangaId, coverUrl: item.cover ? ImageProxy.buildWsrvUrl(item.cover, { quality: 85 }) : null, author: 'Atsumaru', status: 'ONGOING', genres: item.genres || ['Manga'], chapters });
  }
