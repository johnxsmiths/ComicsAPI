...
export class MangaHereScraper extends BaseScraper {
  constructor() {
    super({ siteId: 'mangahere', siteName: 'MangaHere', baseUrl: 'https://mangahere.cc', requiresHeadless: false });
  }

  async search(query) {
    try {
      const html = await this.fetchUrl('/search', { params: { title: query } }).catch(() => null);
      if (!html) return [];
      const $ = cheerio.load(html);
      const items = [];
      $('.manga_text, .manga_pic').each((_, el) => {
  ...
  async getDetails(mangaId) {
    ...
    const $ = cheerio.load(html);
    const title = $('.detail-info-right-title').text().trim() || mangaId;
    const cover = $('.detail-info-cover-img').attr('src');
    const chapters = [];
    $('.detail-main-list li a').each((_, el) => {
    ...
    return formatMangaDetails({ id: mangaId, siteId: this.siteId, title, coverUrl: cover ? ImageProxy.buildWsrvUrl(cover, { quality: 85 }) : null, author: 'MangaHere', status: 'ONGOING', genres: ['Manga'], chapters });
  }
