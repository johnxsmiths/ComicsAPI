...
export class MangaFireScraper extends BaseScraper {
  constructor() {
    super({ siteId: 'mangafire', siteName: 'MangaFire', baseUrl: 'https://mangafire.to', requiresHeadless: true });
  }

  async search(query) {
    try {
      const html = await this.fetchUrl('/filter', { params: { keyword: query } }).catch(() => null);
      if (!html) return [];
      ...
      $('.unit, .inner').each((_, el) => {
        ...
        const title = $(el).find('.name, a.title').text().trim();
  ...
  async getDetails(mangaId) {
    ...
    return formatMangaDetails({ id: mangaId, siteId: this.siteId, title, coverUrl: cover ? ImageProxy.buildWsrvUrl(cover, { quality: 85 }) : null, author: 'MangaFire', status: 'ONGOING', genres: ['Manga'], chapters });
  }
  ...
  async getChapterPages(mangaId, chapterId) {
    ...
    const $ = cheerio.load(html);
    const pages = [];
    $('#reader img, .page img').each((idx, el) => {
