...
/**
 * Scraper implementation for WeebCentral (https://weebcentral.com / formerly MangaSee).
 */
export class WeebCentralScraper extends BaseScraper {
  constructor() {
    super({
      siteId: 'weebcentral',
      siteName: 'WeebCentral',
  ...
  /**
   * Search WeebCentral catalog.
   * @param {string} query
   * @returns {Promise<Array<import('../../models/manga.model.js').SearchResultItem>>}
   */
  async search(query) {
    try {
      const html = await this.httpClient.get('/search/data', {
        params: { author: '', text: query, sort: 'Best Match', order: 'Descending' },
      ...
      $('a[href*="/series/"]').each((_, el) => {
        ...
        const rawCoverUrl = $(el).find('img').attr('src') || $(el).closest('article').find('img').attr('src');

        const mangaId = href.split('/series/')[1]?.split('/')[0] || href;

        if (mangaId && title && title !== 'Official' && !results.some(r => r.id === mangaId)) {
      ...
      throw new Error(`[WeebCentral Scraper] Search failed for query "${query}": ${error.message}`);
    }
  }

  /**
   * Load metadata and chapter list for WeebCentral ID.
   * @param {string} mangaId
   * @returns {Promise<import('../../models/manga.model.js').MangaDetails>}
   */
  async getDetails(mangaId) {
    try {
      const targetUrl = mangaId.startsWith('http') ? mangaId : `/series/${mangaId}`;
      const html = await this.httpClient.get(targetUrl).then(r => r.data);
      const $ = cheerio.load(html);

      const title = $('h1').first().text().trim() || mangaId;
      const description = $('p[class*="description"]').text().trim();
      const rawCoverUrl = $('img[src*="cover"]').first().attr('src');
      ...
      throw new Error(`[WeebCentral Scraper] Failed to fetch details for "${mangaId}": ${error.message}`);
    }
  }
  ...
  async getChapterPages(mangaId, chapterId) {
    try {
      const targetUrl = chapterId.startsWith('http') ? chapterId : `/chapters/${chapterId}/images?reading_style=long_strip`;
      const html = await this.httpClient.get(targetUrl).then(r => r.data);
      const $ = cheerio.load(html);

      const pages = [];
      $('img[src*="planeptune"], img[src*="cdn"]').each((idx, el) => {
      ...
      throw new Error(`[WeebCentral Scraper] Failed to fetch pages for "${chapterId}": ${error.message}`);
    }
  }
}
